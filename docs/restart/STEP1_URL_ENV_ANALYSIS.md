...
```
Frontend → Backend API calls via NEXT_PUBLIC_API_BASE:
...
### Mapping: Current → Standard Env Vars
...
Standard: SUPABASE_URL, SUPABASE_ANON_KEY (✅ already matches)
```
...
```
Current: NEXT_PUBLIC_API_BASE (✅ already standard)
Standard: NEXT_PUBLIC_API_BASE
```

**In frontend Supabase init**
```
Current: NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY (✅ already standard)
...
# Found in Supabase dashboard under Settings → API → Reveal
...
- **Status:** ✅ GOOD (uses env var with local fallback)
...
- **Status:** ⚠️ NEEDS FIX - Remove `/api` suffix to match api.ts
...
- **Status:** ✅ GOOD (uses env var)
- **Line 112:** `const appOrigin = typeof window !== 'undefined' ? window.location.origin : 'http://localhost:4000';`
- **Status:** ✅ ACCEPTABLE (fallback only used on server side, client always has window.location)
...
- **Status:** ⚠️ REPEATED PATTERN (should extract to shared util)
...
- **Status:** 🔴 CRITICAL - Must use FRONTEND_URL env var
...
- **Status:** 🔴 CRITICAL - Must use env var for FRONTEND_URL
...
- **Status:** ⚠️ DOCUMENTATION (not critical for URL consistency)
...
```
vizpilot/
├── backend/                    # OLD LOCATION - Has .env.example
│   ├── .env.example
│   ├── app/
│   ├── check_env.py
│   └── ...
├── frontend/                   # MIXED LOCATION - Has some files
│   ├── .env.example
│   ├── .env.local
│   ├── vercel.json
│   └── ...
├── elas-erp/                   # NEW LOCATION
│   ├── backend/                # ACTUAL BACKEND - Has .env
│   │   ├── .env
│   │   ├── app/
│   │   ├── check_env.py
│   │   └── ...
  ...
  export function validateFrontendEnv() {
    ...
    if (errors.length > 0) {
      const message = [
        ...
        '',
        '🔗 Get Supabase credentials from: https://app.supabase.com/project/[id]/settings/api'
      ].join('\n');
